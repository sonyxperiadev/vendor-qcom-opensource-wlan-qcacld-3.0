						  ...
						  bool append_ie)
{
	...
	case 4:
		...
		pBuf += 4;
		ielen -= 4;
		pDst->tbtt_info.tbtt_info_4.mld_id = tmp116__ >> 0 & 0xff;
		pDst->tbtt_info.tbtt_info_4.link_id = tmp116__ >> 8 & 0xf;
		pDst->tbtt_info.tbtt_info_4.bss_param_change_cnt = tmp116__ >> 12 & 0xff;
		pDst->tbtt_info.tbtt_info_4.reserved = tmp116__ >> 20 & 0xfff;
		break;
	case 5:
		if (unlikely(ielen < 1)) {
			pDst->present = 0;
	...
	case 10:
		...
		pBuf += 4;
		ielen -= 4;
		pDst->tbtt_info.tbtt_info_10.mld_id = tmp117__ >> 0 & 0xff;
		pDst->tbtt_info.tbtt_info_10.link_id = tmp117__ >> 8 & 0xf;
		pDst->tbtt_info.tbtt_info_10.bss_param_change_cnt = tmp117__ >> 12 & 0xff;
		pDst->tbtt_info.tbtt_info_10.reserved = tmp117__ >> 20 & 0xfff;
		break;
	case 11:
		if (unlikely(ielen < 1)) {
			pDst->present = 0;
	...
	case 16:
		...
		framesntohl(pCtx, &tmp118__, pBuf, 0);
		pBuf += 4;
		ielen -= 4;
		pDst->tbtt_info.tbtt_info_16.mld_id = tmp118__ >> 0 & 0xff;
		pDst->tbtt_info.tbtt_info_16.link_id = tmp118__ >> 8 & 0xf;
		pDst->tbtt_info.tbtt_info_16.bss_param_change_cnt = tmp118__ >> 12 & 0xff;
		pDst->tbtt_info.tbtt_info_16.reserved = tmp118__ >> 20 & 0xfff;
		break;
	}
	(void)pCtx;
	return status;
				 ...
				 uint8_t *pBuf)
{
	...
	tmp119__ = 0U;
	tmp119__ |= (pSrc->ess << 0);
	tmp119__ |= (pSrc->ibss << 1);
	tmp119__ |= (pSrc->cfPollable << 2);
	tmp119__ |= (pSrc->cfPollReq << 3);
	tmp119__ |= (pSrc->privacy << 4);
	tmp119__ |= (pSrc->shortPreamble << 5);
	tmp119__ |= (pSrc->criticalUpdateFlag << 6);
	tmp119__ |= (pSrc->channelAgility << 7);
	tmp119__ |= (pSrc->spectrumMgt << 8);
	tmp119__ |= (pSrc->qos << 9);
	tmp119__ |= (pSrc->shortSlotTime << 10);
	tmp119__ |= (pSrc->apsd << 11);
	tmp119__ |= (pSrc->rrm << 12);
	tmp119__ |= (pSrc->dsssOfdm << 13);
	tmp119__ |= (pSrc->delayedBA << 14);
	tmp119__ |= (pSrc->immediateBA << 15);
				  ...
				  uint8_t *pBuf)
{
	uint8_t tmp120__;
	tmp120__ = 0U;
	tmp120__ |= (pSrc->chanWidth << 0);
	tmp120__ |= (pSrc->reserved << 2);
	tmp120__ |= (pSrc->rxNSS << 4);
	tmp120__ |= (pSrc->rxNSSType << 7);
	*pBuf = tmp120__;
	(void)pCtx;
				   ...
				   uint8_t *pBuf)
{
	uint8_t tmp121__;
	tmp121__ = 0U;
	tmp121__ |= (pSrc->PowerSave_En << 0);
	tmp121__ |= (pSrc->Mode << 1);
			   ...
			   uint8_t *pBuf)
{
	uint32_t tmp122__;
	tmp122__ = 0U;
	tmp122__ |= (pSrc->traffic_type << 0);
	tmp122__ |= (pSrc->tsid << 1);
	tmp122__ |= (pSrc->direction << 5);
	tmp122__ |= (pSrc->access_policy << 7);
	tmp122__ |= (pSrc->aggregation << 9);
	tmp122__ |= (pSrc->psb << 10);
	tmp122__ |= (pSrc->user_priority << 11);
	tmp122__ |= (pSrc->tsinfo_ack_pol << 14);
	tmp122__ |= (pSrc->schedule << 16);
	tmp122__ |= (pSrc->unused << 17);
				    ...
				    uint8_t *pBuf)
{
	uint16_t tmp123__;
	tmp123__ = 0U;
	tmp123__ |= (pSrc->amsdu_supp << 0);
	tmp123__ |= (pSrc->policy << 1);
	tmp123__ |= (pSrc->tid << 2);
	tmp123__ |= (pSrc->buff_size << 6);
				    ...
				    uint8_t *pBuf)
{
	...
	tmp125__ |= (pSrc->initiator << 11);
	tmp125__ |= (pSrc->tid << 12);
					       ...
					       uint8_t *pBuf)
{
	uint32_t tmp126__;
	tmp126__ = 0U;
	tmp126__ |= (pSrc->switch_mode << 0);
	tmp126__ |= (pSrc->op_class << 8);
	tmp126__ |= (pSrc->new_channel << 16);
	tmp126__ |= (pSrc->switch_count << 24);
				 ...
				 uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		pBuf += 2; *pnConsumed += 2;
		tmp128__ = 0U;
		tmp128__ |= (pSrc->minor << 0);
		tmp128__ |= (pSrc->major << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
				 ...
				 uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		*pBuf = 101;
		...
		pBuf += 1; *pnConsumed += 1;
		tmp129__ = 0U;
		tmp129__ |= (pSrc->oce_release << 0);
		tmp129__ |= (pSrc->is_sta_cfon << 3);
		tmp129__ |= (pSrc->non_oce_ap_present << 4);
					     ...
					     uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		*pBuf = 103;
		...
		pBuf += 1; *pnConsumed += 1;
		tmp130__ = 0U;
		tmp130__ |= (pSrc->downlink_av_cap << 0);
		tmp130__ |= (pSrc->uplink_av_cap << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
			    ...
			    uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 2;
		...
		++pBuf; ++(*pnConsumed);
		tmp131__ = 0U;
		tmp131__ |= (pSrc->keyId << 0);
		tmp131__ |= (pSrc->reserved << 2);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
				      ...
				      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 70;
		...
		++pBuf; ++(*pnConsumed);
		tmp132__ = 0U;
		tmp132__ |= (pSrc->LinkMeasurement << 0);
		tmp132__ |= (pSrc->NeighborRpt << 1);
		tmp132__ |= (pSrc->parallel << 2);
		tmp132__ |= (pSrc->repeated << 3);
		tmp132__ |= (pSrc->BeaconPassive << 4);
		tmp132__ |= (pSrc->BeaconActive << 5);
		tmp132__ |= (pSrc->BeaconTable << 6);
		tmp132__ |= (pSrc->BeaconRepCond << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp133__ = 0U;
		tmp133__ |= (pSrc->FrameMeasurement << 0);
		tmp133__ |= (pSrc->ChannelLoad << 1);
		tmp133__ |= (pSrc->NoiseHistogram << 2);
		tmp133__ |= (pSrc->statistics << 3);
		tmp133__ |= (pSrc->LCIMeasurement << 4);
		tmp133__ |= (pSrc->LCIAzimuth << 5);
		tmp133__ |= (pSrc->TCMCapability << 6);
		tmp133__ |= (pSrc->triggeredTCM << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp134__ = 0U;
		tmp134__ |= (pSrc->APChanReport << 0);
		tmp134__ |= (pSrc->RRMMIBEnabled << 1);
		tmp134__ |= (pSrc->operatingChanMax << 2);
		tmp134__ |= (pSrc->nonOperatinChanMax << 5);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp135__ = 0U;
		tmp135__ |= (pSrc->MeasurementPilot << 0);
		tmp135__ |= (pSrc->MeasurementPilotEnabled << 3);
		tmp135__ |= (pSrc->NeighborTSFOffset << 4);
		tmp135__ |= (pSrc->RCPIMeasurement << 5);
		tmp135__ |= (pSrc->RSNIMeasurement << 6);
		tmp135__ |= (pSrc->BssAvgAccessDelay << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp136__ = 0U;
		tmp136__ |= (pSrc->BSSAvailAdmission << 0);
		tmp136__ |= (pSrc->AntennaInformation << 1);
		tmp136__ |= (pSrc->fine_time_meas_rpt << 2);
		tmp136__ |= (pSrc->lci_capability << 3);
			      ...
			      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 13;
		...
		++pBuf; ++(*pnConsumed);
		tmp138__ = 0U;
		tmp138__ |= (pSrc->traffic_type << 0);
		tmp138__ |= (pSrc->tsid << 1);
		tmp138__ |= (pSrc->direction << 5);
		tmp138__ |= (pSrc->access_policy << 7);
		tmp138__ |= (pSrc->aggregation << 9);
		tmp138__ |= (pSrc->psb << 10);
		tmp138__ |= (pSrc->user_priority << 11);
		tmp138__ |= (pSrc->tsinfo_ack_pol << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		nBuf -=  2 ;
		tmp139__ = 0U;
		tmp139__ |= (pSrc->schedule << 0);
		tmp139__ |= (pSrc->unused << 1);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp140__ = 0U;
		tmp140__ |= (pSrc->size << 0);
		tmp140__ |= (pSrc->fixed << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
				...
				uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 191;
		...
		tmp141__ |= (pSrc->extended_nss_bw_supp << 30);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		tmp142__ |= (pSrc->max_nsts_total << 13);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		tmp143__ |= (pSrc->vht_extended_nss_bw_cap << 13);
		tmp143__ |= (pSrc->reserved << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
				 ...
				 uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pnConsumed += 2;
		...
		nBuf -=  2 ;
		tmp146__ = 0U;
		tmp146__ |= (pSrc->tsinfo_rsvd << 0);
		tmp146__ |= (pSrc->burst_size_defn << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
							   ...
							   uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 2;
		...
		++pBuf; ++(*pnConsumed);
		tmp148__ = 0U;
		tmp148__ |= (pSrc->beacon_report_id << 0);
		tmp148__ |= (pSrc->fragment_id_number << 8);
		tmp148__ |= (pSrc->more_fragments << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
			       ...
			       uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 106;
		++pBuf; ++(*pnConsumed);
		tmp205__ = 0U;
		tmp205__ |= (pSrc->width << 0);
		tmp205__ |= (pSrc->reserved << 3);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		pBuf += 2;
		tmp206__ = 0U;
		tmp206__ |= (pSrc->disable_sub_chan_bitmap_present << 0);
		tmp206__ |= (pSrc->reserved2 << 1);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
					...
					uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 240;
		...
		++pBuf; ++(*pnConsumed);
		tmp207__ = 0U;
		tmp207__ |= (pSrc->public_key_identifiers_cnt << 0);
		tmp207__ |= (pSrc->realm_identifiers_cnt << 3);
		tmp207__ |= (pSrc->is_ip_config_supported << 6);
		tmp207__ |= (pSrc->is_cache_id_present << 7);
		tmp207__ |= (pSrc->is_hessid_present << 8);
		tmp207__ |= (pSrc->is_fils_sk_auth_supported << 9);
		tmp207__ |= (pSrc->is_fils_sk_auth_pfs_supported << 10);
		tmp207__ |= (pSrc->is_pk_auth_supported << 11);
		tmp207__ |= (pSrc->reserved << 12);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
					 ...
					 uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 59;
		++pBuf; ++(*pnConsumed);
		tmp208__ = 0U;
		tmp208__ |= (pSrc->min_mpdu_start_spacing << 0);
		tmp208__ |= (pSrc->max_ampdu_len_exp << 3);
		tmp208__ |= (pSrc->max_mpdu_len << 6);
		tmp208__ |= (pSrc->sm_pow_save << 9);
		tmp208__ |= (pSrc->rd_responder << 11);
		tmp208__ |= (pSrc->rx_ant_pattern_consistency << 12);
		tmp208__ |= (pSrc->tx_ant_pattern_consistency << 13);
		tmp208__ |= (pSrc->reserved << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
			       ...
			       uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 35;
		++pBuf; ++(*pnConsumed);
		tmp209__ = 0U;
		tmp209__ |= (pSrc->htc_he << 0);
		tmp209__ |= (pSrc->twt_request << 1);
		tmp209__ |= (pSrc->twt_responder << 2);
		tmp209__ |= (pSrc->fragmentation << 3);
		tmp209__ |= (pSrc->max_num_frag_msdu_amsdu_exp << 5);
		tmp209__ |= (pSrc->min_frag_size << 8);
		tmp209__ |= (pSrc->trigger_frm_mac_pad << 10);
		tmp209__ |= (pSrc->multi_tid_aggr_rx_supp << 12);
		tmp209__ |= (pSrc->he_link_adaptation << 15);
		tmp209__ |= (pSrc->all_ack << 17);
		tmp209__ |= (pSrc->trigd_rsp_sched << 18);
		tmp209__ |= (pSrc->a_bsr << 19);
		tmp209__ |= (pSrc->broadcast_twt << 20);
		tmp209__ |= (pSrc->ba_32bit_bitmap << 21);
		tmp209__ |= (pSrc->mu_cascade << 22);
		tmp209__ |= (pSrc->ack_enabled_multitid << 23);
		tmp209__ |= (pSrc->reserved << 24);
		tmp209__ |= (pSrc->omi_a_ctrl << 25);
		tmp209__ |= (pSrc->ofdma_ra << 26);
		tmp209__ |= (pSrc->max_ampdu_len_exp_ext << 27);
		tmp209__ |= (pSrc->amsdu_frag << 29);
		tmp209__ |= (pSrc->flex_twt_sched << 30);
		tmp209__ |= (pSrc->rx_ctrl_frame << 31);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 4;
		...
		nBuf -=  4 ;
		tmp210__ = 0U;
		tmp210__ |= (pSrc->bsrp_ampdu_aggr << 0);
		tmp210__ |= (pSrc->qtp << 1);
		tmp210__ |= (pSrc->a_bqr << 2);
		tmp210__ |= (pSrc->spatial_reuse_param_rspder << 3);
		tmp210__ |= (pSrc->ndp_feedback_supp << 4);
		tmp210__ |= (pSrc->ops_supp << 5);
		tmp210__ |= (pSrc->amsdu_in_ampdu << 6);
		tmp210__ |= (pSrc->multi_tid_aggr_tx_supp << 7);
		tmp210__ |= (pSrc->he_sub_ch_sel_tx_supp << 10);
		tmp210__ |= (pSrc->ul_2x996_tone_ru_supp << 11);
		tmp210__ |= (pSrc->om_ctrl_ul_mu_data_dis_rx << 12);
		tmp210__ |= (pSrc->he_dynamic_smps << 13);
		tmp210__ |= (pSrc->punctured_sounding_supp << 14);
		tmp210__ |= (pSrc->ht_vht_trg_frm_rx_supp << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		nBuf -=  2 ;
		tmp211__ = 0U;
		tmp211__ |= (pSrc->reserved2 << 0);
		tmp211__ |= (pSrc->chan_width_0 << 1);
		tmp211__ |= (pSrc->chan_width_1 << 2);
		tmp211__ |= (pSrc->chan_width_2 << 3);
		tmp211__ |= (pSrc->chan_width_3 << 4);
		tmp211__ |= (pSrc->chan_width_4 << 5);
		tmp211__ |= (pSrc->chan_width_5 << 6);
		tmp211__ |= (pSrc->chan_width_6 << 7);
		tmp211__ |= (pSrc->rx_pream_puncturing << 8);
		tmp211__ |= (pSrc->device_class << 12);
		tmp211__ |= (pSrc->ldpc_coding << 13);
		tmp211__ |= (pSrc->he_1x_ltf_800_gi_ppdu << 14);
		tmp211__ |= (pSrc->midamble_tx_rx_max_nsts << 15);
		tmp211__ |= (pSrc->he_4x_ltf_3200_gi_ndp << 17);
		tmp211__ |= (pSrc->tb_ppdu_tx_stbc_lt_80mhz << 18);
		tmp211__ |= (pSrc->rx_stbc_lt_80mhz << 19);
		tmp211__ |= (pSrc->doppler << 20);
		tmp211__ |= (pSrc->ul_mu << 22);
		tmp211__ |= (pSrc->dcm_enc_tx << 24);
		tmp211__ |= (pSrc->dcm_enc_rx << 27);
		tmp211__ |= (pSrc->ul_he_mu << 30);
		tmp211__ |= (pSrc->su_beamformer << 31);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;

		frameshtonl(pCtx, pBuf, tmp211__, 0);
		*pnConsumed += 4;
		pBuf += 4;
		nBuf -=  4 ;
		tmp212__ = 0U;
		tmp212__ |= (pSrc->su_beamformee << 0);
		tmp212__ |= (pSrc->mu_beamformer << 1);
		tmp212__ |= (pSrc->bfee_sts_lt_80 << 2);
		tmp212__ |= (pSrc->bfee_sts_gt_80 << 5);
		tmp212__ |= (pSrc->num_sounding_lt_80 << 8);
		tmp212__ |= (pSrc->num_sounding_gt_80 << 11);
		tmp212__ |= (pSrc->su_feedback_tone16 << 14);
		tmp212__ |= (pSrc->mu_feedback_tone16 << 15);
		tmp212__ |= (pSrc->codebook_su << 16);
		tmp212__ |= (pSrc->codebook_mu << 17);
		tmp212__ |= (pSrc->beamforming_feedback << 18);
		tmp212__ |= (pSrc->he_er_su_ppdu << 21);
		tmp212__ |= (pSrc->dl_mu_mimo_part_bw << 22);
		tmp212__ |= (pSrc->ppet_present << 23);
		tmp212__ |= (pSrc->srp << 24);
		tmp212__ |= (pSrc->power_boost << 25);
		tmp212__ |= (pSrc->he_ltf_800_gi_4x << 26);
		tmp212__ |= (pSrc->max_nc << 27);
		tmp212__ |= (pSrc->tb_ppdu_tx_stbc_gt_80mhz << 30);
		tmp212__ |= (pSrc->rx_stbc_gt_80mhz << 31);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 4;
		...
		nBuf -=  4 ;
		tmp213__ = 0U;
		tmp213__ |= (pSrc->er_he_ltf_800_gi_4x << 0);
		tmp213__ |= (pSrc->he_ppdu_20_in_40Mhz_2G << 1);
		tmp213__ |= (pSrc->he_ppdu_20_in_160_80p80Mhz << 2);
		tmp213__ |= (pSrc->he_ppdu_80_in_160_80p80Mhz << 3);
		tmp213__ |= (pSrc->er_1x_he_ltf_gi << 4);
		tmp213__ |= (pSrc->midamble_tx_rx_1x_he_ltf << 5);
		tmp213__ |= (pSrc->dcm_max_bw << 6);
		tmp213__ |= (pSrc->longer_than_16_he_sigb_ofdm_sym << 8);
		tmp213__ |= (pSrc->non_trig_cqi_feedback << 9);
		tmp213__ |= (pSrc->tx_1024_qam_lt_242_tone_ru << 10);
		tmp213__ |= (pSrc->rx_1024_qam_lt_242_tone_ru << 11);
		tmp213__ |= (pSrc->rx_full_bw_su_he_mu_compress_sigb << 12);
		tmp213__ |= (pSrc->rx_full_bw_su_he_mu_non_cmpr_sigb << 13);
		tmp213__ |= (pSrc->reserved3 << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
			      ...
			      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 36;
		++pBuf; ++(*pnConsumed);
		tmp214__ = 0U;
		tmp214__ |= (pSrc->default_pe << 0);
		tmp214__ |= (pSrc->twt_required << 3);
		tmp214__ |= (pSrc->txop_rts_threshold << 4);
		tmp214__ |= (pSrc->vht_oper_present << 14);
		tmp214__ |= (pSrc->co_located_bss << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		nBuf -=  2 ;
		tmp215__ = 0U;
		tmp215__ |= (pSrc->er_su_disable << 0);
		tmp215__ |= (pSrc->oper_info_6g_present << 1);
		tmp215__ |= (pSrc->reserved2 << 2);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp216__ = 0U;
		tmp216__ |= (pSrc->bss_color << 0);
		tmp216__ |= (pSrc->partial_bss_col << 6);
		tmp216__ |= (pSrc->bss_col_disabled << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		if (pSrc->oper_info_6g_present) {
			...
			case 1:
				...
				*pnConsumed += 1;
				pBuf += 1;
				tmp217__ = 0U;
				tmp217__ |= (pSrc->oper_info_6g.info.ch_width << 0);
				tmp217__ |= (pSrc->oper_info_6g.info.dup_bcon << 2);
				tmp217__ |= (pSrc->oper_info_6g.info.reg_info << 3);
				tmp217__ |= (pSrc->oper_info_6g.info.reserved << 6);
				if (unlikely(nBuf < 1))
					return DOT11F_INCOMPLETE_IE;
				      ...
				      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 0x10;
		++pBuf; ++(*pnConsumed);
		tmp218__ = 0U;
		tmp218__ |= (pSrc->dgaf_dis << 0);
		tmp218__ |= (pSrc->hs_id_present << 1);
		tmp218__ |= (pSrc->reserved << 3);
		tmp218__ |= (pSrc->release_num << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
					       ...
					       uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 72;
		...
		++pBuf; ++(*pnConsumed);
		tmp219__ = 0U;
		tmp219__ |= (pSrc->info_request << 0);
		tmp219__ |= (pSrc->forty_mhz_intolerant << 1);
		tmp219__ |= (pSrc->twenty_mhz_bsswidth_req << 2);
		tmp219__ |= (pSrc->obss_scan_exemption_req << 3);
		tmp219__ |= (pSrc->obss_scan_exemption_grant << 4);
		tmp219__ |= (pSrc->unused << 5);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
	...
	while (pSrc->present) {
		...
		*pnConsumed += 2;
		...
		if (pSrc->link_id_info_present) {
			...
			case 1:
				tmp221__ = 0U;
				tmp221__ |= (pSrc->link_id_info.info.link_id << 0);
				tmp221__ |= (pSrc->link_id_info.info.reserved << 4);
				if (unlikely(nBuf < 1))
					return DOT11F_INCOMPLETE_IE;
		...
		if (pSrc->medium_sync_delay_info_present) {
			...
			case 1:
				tmp222__ = 0U;
				tmp222__ |= (pSrc->medium_sync_delay_info.info.medium_sync_duration << 0);
				tmp222__ |= (pSrc->medium_sync_delay_info.info.medium_sync_ofdm_ed_thresh << 8);
				tmp222__ |= (pSrc->medium_sync_delay_info.info.medium_sync_max_txop_num << 12);
				if (unlikely(nBuf < 2))
					return DOT11F_INCOMPLETE_IE;
		...
		if (pSrc->eml_capab_present) {
			...
			case 1:
				...
				*pnConsumed += 2;
				...
				nBuf -=  2 ;
				tmp224__ = 0U;
				tmp224__ |= (pSrc->eml_capabilities.info.emlmr_rx_nss << 0);
				tmp224__ |= (pSrc->eml_capabilities.info.emlmr_tx_nss << 4);
				if (unlikely(nBuf < 1))
					return DOT11F_INCOMPLETE_IE;
		...
		if (pSrc->mld_capab_present) {
			...
			case 1:
				tmp225__ = 0U;
				tmp225__ |= (pSrc->mld_capabilities.info.max_simultaneous_link_num << 0);
					  ...
					  uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp226__ = 0U;
		tmp226__ |= (pSrc->acbe_aifsn << 0);
		tmp226__ |= (pSrc->acbe_acm << 4);
		tmp226__ |= (pSrc->acbe_aci << 5);
		tmp226__ |= (pSrc->unused1 << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp227__ = 0U;
		tmp227__ |= (pSrc->acbe_acwmin << 0);
		tmp227__ |= (pSrc->acbe_acwmax << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp228__ = 0U;
		tmp228__ |= (pSrc->acbk_aifsn << 0);
		tmp228__ |= (pSrc->acbk_acm << 4);
		tmp228__ |= (pSrc->acbk_aci << 5);
		tmp228__ |= (pSrc->unused2 << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp229__ = 0U;
		tmp229__ |= (pSrc->acbk_acwmin << 0);
		tmp229__ |= (pSrc->acbk_acwmax << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp230__ = 0U;
		tmp230__ |= (pSrc->acvi_aifsn << 0);
		tmp230__ |= (pSrc->acvi_acm << 4);
		tmp230__ |= (pSrc->acvi_aci << 5);
		tmp230__ |= (pSrc->unused3 << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp231__ = 0U;
		tmp231__ |= (pSrc->acvi_acwmin << 0);
		tmp231__ |= (pSrc->acvi_acwmax << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp232__ = 0U;
		tmp232__ |= (pSrc->acvo_aifsn << 0);
		tmp232__ |= (pSrc->acvo_acm << 4);
		tmp232__ |= (pSrc->acvo_aci << 5);
		tmp232__ |= (pSrc->unused4 << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp233__ = 0U;
		tmp233__ |= (pSrc->acvo_acwmin << 0);
		tmp233__ |= (pSrc->acvo_acwmax << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
						...
						uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 201;
		...
		++pBuf; ++(*pnConsumed);
		tmp234__ = 0U;
		tmp234__ |= (pSrc->tbtt_type << 0);
		tmp234__ |= (pSrc->filtered_neighbor_ap << 2);
		tmp234__ |= (pSrc->reserved << 3);
		tmp234__ |= (pSrc->tbtt_info_count << 4);
		tmp234__ |= (pSrc->tbtt_info_len << 8);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;

		frameshtons(pCtx, pBuf, tmp234__, 0);
		*pnConsumed += 2;
		pBuf += 2;
		nBuf -=  2 ;
		*pBuf = pSrc->op_class;
		...
		*pnConsumed += 1;
		...
		case 4:
			...
			*pnConsumed += 1;
			pBuf += 1;
			tmp235__ = 0U;
			tmp235__ |= (pSrc->tbtt_info.tbtt_info_4.mld_id << 0);
			tmp235__ |= (pSrc->tbtt_info.tbtt_info_4.link_id << 8);
			tmp235__ |= (pSrc->tbtt_info.tbtt_info_4.bss_param_change_cnt << 12);
			tmp235__ |= (pSrc->tbtt_info.tbtt_info_4.reserved << 20);
			if (unlikely(nBuf < 4))
				return DOT11F_INCOMPLETE_IE;
		...
		case 10:
			...
			*pnConsumed += 6;
			pBuf += 6;
			tmp236__ = 0U;
			tmp236__ |= (pSrc->tbtt_info.tbtt_info_10.mld_id << 0);
			tmp236__ |= (pSrc->tbtt_info.tbtt_info_10.link_id << 8);
			tmp236__ |= (pSrc->tbtt_info.tbtt_info_10.bss_param_change_cnt << 12);
			tmp236__ |= (pSrc->tbtt_info.tbtt_info_10.reserved << 20);
			if (unlikely(nBuf < 4))
				return DOT11F_INCOMPLETE_IE;

			frameshtonl(pCtx, pBuf, tmp236__, 0);
			*pnConsumed += 4;
			/* fieldsEndFlag  = 1 */
			nBuf -=  4 ;
			break;
		...
		case 16:
			...
			*pnConsumed += 1;
			pBuf += 1;
			tmp237__ = 0U;
			tmp237__ |= (pSrc->tbtt_info.tbtt_info_16.mld_id << 0);
			tmp237__ |= (pSrc->tbtt_info.tbtt_info_16.link_id << 8);
			tmp237__ |= (pSrc->tbtt_info.tbtt_info_16.bss_param_change_cnt << 12);
			tmp237__ |= (pSrc->tbtt_info.tbtt_info_16.reserved << 20);
			if (unlikely(nBuf < 4))
				return DOT11F_INCOMPLETE_IE;

			frameshtonl(pCtx, pBuf, tmp237__, 0);
			*pnConsumed += 4;
			/* fieldsEndFlag  = 1 */
			nBuf -=  4 ;
			break;
